...
import { 
  IconCrown, IconBuilding, IconPhone, IconMapPin, IconCar, IconUser, 
  IconEdit, IconVolume2, IconVolumeX, IconZap, IconScale, IconCopy, 
  IconCheck, IconClose, IconArrowRight, IconBot, IconCpu 
...
export default function AIDetectionStudio() {
  ...
  const draw = useCallback(() => {
    ...
    visibleNodes.forEach(node => {
      ...
      // Center Monogram
      ctx.font = `bold ${Math.round(radius * 0.75)}px JetBrains Mono, monospace`
      ctx.fillStyle = '#ffffff'
      ctx.textAlign = 'center'
      ctx.textBaseline = 'middle'
      const monogram = node.type === 'kingpin' ? '★' : node.type === 'bank' ? 'B' : node.type === 'phone' ? 'T' : node.type === 'location' ? 'L' : node.type === 'vehicle' ? 'V' : 'P'
      ctx.fillText(monogram, node.x, node.y)
          ...
          <div className="ai-preset-chips">
            ...
            >
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                <IconEdit size={14} /> Custom Officer Input
              </span>
            </button>
          </div>
          ...
          <div className="ai-textarea-wrapper">
            ...
            <div className="ai-textarea-actions">
              <button
                ...
                title="Neural Speech Readout"
                style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}
              >
                {isSpeaking ? (
                  <><IconVolumeX size={14} /> Stop Voice</>
                ) : (
                  <><IconVolume2 size={14} /> Audio Briefing</>
                )}
              </button>
              <button
              ...
              >
                {isProcessing ? (
                  ...
                  <><IconZap size={14} /> Ingest &amp; Run Detection →</>
                )}
              </button>
            </div>
          </div>
        ...
        <div className="ai-studio-grid">
          ...
          <div className="ai-inspector-panel">
              ...
              {selectedNode ? (
                <div className="inspector-content">
                  <div className="node-profile-hdr">
                    <div className="node-avatar-large" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      {selectedNode.type === 'kingpin' ? (
                        <IconCrown size={22} color="var(--purple-l)" />
                      ) : selectedNode.type === 'bank' ? (
                        <IconBuilding size={22} color="var(--cyan-l)" />
                      ) : selectedNode.type === 'phone' ? (
                        <IconPhone size={22} color="var(--green-l)" />
                      ) : selectedNode.type === 'location' ? (
                        <IconMapPin size={22} color="var(--yellow-l)" />
                      ) : selectedNode.type === 'vehicle' ? (
                        <IconCar size={22} color="var(--orange)" />
                      ) : (
                        <IconUser size={22} color="var(--purple-l)" />
                      )}
                    </div>
                    <div>
            ...
            <div className="card-studio consensus-card">
              ...
              <div className="consensus-dialogue">
                ...
                <div className="dialogue-bubble auditor">
                  <div className="bubble-speaker mono accent-green">VALIDATOR AGENT (AUDITOR):</div>
                  <p className="bubble-text">
            ...
            <div className="action-buttons-stack">
              ...
              >
                <IconScale size={14} /> View Sec 65B Court Evidence Dossier →
              </button>
              
              <Link to="/dashboard" className="btn btn-outline btn-sync-dash" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                <IconArrowRight size={14} /> Open in Full Investigation Dashboard
              </Link>
            </div>
        ...
        <div className="dossier-modal-overlay" onClick={() => setDossierOpen(false)}>
          <div className="dossier-modal card-studio" onClick={(e) => e.stopPropagation()}>
            <div className="dossier-header">
              ...
              <button className="btn-close" onClick={() => setDossierOpen(false)} aria-label="Close modal">
                <IconClose size={16} />
              </button>
            </div>
            ...
            <div className="dossier-footer">
              <button className="btn btn-primary" onClick={handleCopyDossier} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                {dossierCopied ? (
                  <><IconCheck size={14} /> Copied to Clipboard!</>
                ) : (
                  <><IconCopy size={14} /> Copy Certified Dossier</>
                )}
              </button>
